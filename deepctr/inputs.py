...
def create_embedding_dict(sparse_feature_columns, varlen_sparse_feature_columns, l2_reg, seed=1024,
...
def embedding_lookup(sparse_embedding_dict, sparse_input_dict, sparse_feature_columns, untrain_feature_columns=[], return_feat_list=(), mask_feat_list=(), to_list=False):
    group_embedding_dict = defaultdict(list)
    untrain_embedding_dict = defaultdict(list)
    for fc in sparse_feature_columns:
        ...
        if (len(return_feat_list) == 0 or feature_name in return_feat_list):
            ...
            emb = sparse_embedding_dict[embedding_name](lookup_idx)
            #print(emb.name, emb.trainable)
            group_embedding_dict[fc.group_name].append(emb)
    for fc in untrain_feature_columns:
        feature_name = fc.name
        embedding_name = fc.embedding_name
        if (len(return_feat_list) == 0 or feature_name in return_feat_list):
            if fc.use_hash:
            ...
            emb = sparse_embedding_dict[embedding_name](lookup_idx)
            emb.trainable = False
            untrain_embedding_dict[fc.group_name].append(emb)
    if to_list:
        return list(chain.from_iterable(group_embedding_dict.values()))
    return group_embedding_dict, untrain_embedding_dict
