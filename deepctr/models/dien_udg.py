...
from ..feature_column import SparseFeat, VarLenSparseFeat, DenseFeat, build_input_features, input_from_feature_columns
from ..inputs import get_varlen_pooling_list, create_embedding_matrix, embedding_lookup, varlen_embedding_lookup, \
    get_dense_input, create_embedding_dict
from ..layers.core import DNN, PredictionLayer, DNN_UDG
...
def DIEN_UDG(dnn_feature_columns, untrain_feature_columns, history_feature_list,
         gru_type="GRU", use_negsampling=False, alpha=1.0, use_bn=False, dnn_hidden_units=(200, 80),
         dnn_activation='relu',
         att_hidden_units=(200, 80), att_activation="dice", att_weight_normalization=True,
         l2_reg_dnn=0, l2_reg_embedding=1e-6, dnn_dropout=0, seed=1024, task='binary', uid_feature_name='',
...
#     _, _, untrain_embedding_dict = input_from_feature_columns(features, dnn_feature_columns, 
...
#                                                               seed, support_group=True)
    ...
    profile_emb_list = dnn_input_emb_list[2:].copy()
    uid_emb_list += profile_emb_list
    udg_embedding_size = (len(profile_emb_list)+1)*udg_embedding_size
    print('uid_emb_list:', uid_emb_list)
    ...
    dnn_input = combined_dnn_input([deep_input_emb], dense_value_list, udg_label=1, udg_embedding_list=uid_emb_list)
    print('?:',dnn_input)
        
    output = DNN_UDG(dnn_hidden_units, dnn_activation, l2_reg_dnn,
                 dnn_dropout, use_bn, seed, udg_embedding_size=udg_embedding_size)(dnn_input)
    ...
    if str(model_input_list[6].name).__contains__('seq_len'):
        del(model_input_list[6])
