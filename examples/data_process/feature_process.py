...
def negative_sampling(df, sequence_features, features_type='amazon_books', path='../data_cache/',
    ...
    if features_type is 'amazon_books':
        ...
        items_info = pd.read_csv('../dataset/alimama/a.csv')
        ...
        for col in tqdm(['itemId', 'category'], desc='encoder for items_side(neg_sample_taobao)' + '_' + vv):
            cache_sf = {'UNK': 0}
            idx = 1
        ...
        if is_training:
            ...
            for i in tqdm(range(len(items_info)), desc='extract ' + behavior_features1 + '2' + behavior_features2):
                if str(items_info['itemId'][i]) not in mid_cat:
                    mid_cat[str(items_info['itemId'][i])] = str(items_info['category'][i])
            with open('./data_cache/mid2cat.json', 'w') as f:
                json.dump(mid_cat, f)
        else:
            with open('./data_cache/mid2cat.json', 'r') as f:
        ...
        df['neg_' + sequence_features[0]] = df[sequence_features[0]].progress_map(run)
        df['neg_' + sequence_features[1]] = df['neg_' + sequence_features[0]].progress_map(
            lambda x: list(map(lambda y: mid_cat.get(y, 'UNK'), x)))

        df['neg_' + sequence_features[0]] = df['neg_' + sequence_features[0]].progress_map(list2str)
        df['neg_' + sequence_features[1]] = df['neg_' + sequence_features[1]].progress_map(list2str)
...
def encoder_sequence(df, sequence_features, features_type='taobao', path='./data_cache/',
                     behavior_features1='itemId', behavior_features2='category', sequence_length=20):
    """
    :param df:
    :param sequence_features:
    :param features_type:
    ...
    if features_type is 'amazon_books':
        ...
        for col in tqdm(['itemId', 'category'], desc='encoder for items_side(encoder_sequence_taobao)'):
            cache_sf = {'UNK': 0}
            idx = 1
            for value in items_info[col]:
                if value not in cache_sf:
